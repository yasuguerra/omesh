...
from ..data.ga_client import query_ga
...
from ..data.data_processing import get_funnel_data

def layout() -> html.Div:
    return html.Div([
        dcc.DatePickerRange(id="date-picker", display_format="YYYY-MM-DD"),
        dcc.Tabs(id="google-subtabs", value="overview_ga", children=[
            dcc.Tab(label="GA Overview", value="overview_ga"),
            dcc.Tab(label="Demography & Geo", value="demography_ga"),
            dcc.Tab(label="Funnels & Paths", value="funnels_ga"),
            dcc.Tab(label="Temporal Analysis", value="temporal_ga"),
            dcc.Tab(label="Correlations", value="correlations_ga"),
            dcc.Tab(label="Cohort Analysis", value="cohort_ga"),
            dcc.Tab(label="What If", value="what_if_ga"),
        ]),
        html.Div(id="google-subtabs-content"),
    ])
...
def register_callbacks(app):
    ...
    def render_google_subtab_content(subtab_ga, start_date, end_date):
        ...
        ai_insight_text = "Insufficient data para un análisis detallado."
        default_no_data_ai_text = "Insufficient data para un análisis detallado."
        ...
        if subtab_ga == 'overview_ga':
            ...
            if df_acq.empty: return html.Div([html.P("No data para la GA Overview."), create_ai_insight_card('overview-ga-ai-insight-visible'), html.Div(default_no_data_ai_text, id='overview-ga-ai-insight-data', style={'display':'none'})])
            ...
            context_overview_ga = f"Resumen GA Overview: Sesiones totales: {df_acq['sessions'].sum():,}. Usuarios totales: {df_acq['Usuarios'].sum():,}. Conversiones totales: {df_acq['conversions'].sum():,}. Tasa de conversión promedio: {df_acq['Tasa Conversion'].mean():.2f}%."
            ...
            return html.Div([
                ...
                create_ai_insight_card('overview-ga-ai-insight-visible', title="💡 Insight Diagnóstico y Acción Action (GA Overview)"),
            ...
            if df_geo.empty:
                geo_opportunities_content.append(html.P("No data geográficos."))
            else:
            ...
            return html.Div([
                html.H4("Analysis Demográfico y Segmentación 🌍📍", className="text-center mt-4"),
                dbc.Row(demographics_graphs_content) if demographics_graphs_content else html.P("No data demográficos suficientes.", className="text-center"),
                html.Hr(className="my-4"),
                html.Div(geo_opportunities_content),
                create_ai_insight_card('demography-ga-ai-insight-visible', title="💡 Insight Diagnóstico y Acción Action (Demography Demografía & Geo Geo)"),
            ...
            kpi_content = html.P("No data de eventos.")
            ...
            sankey_content = [html.H4("Analysis de Rutas (Sankey)", className="mt-5 text-center")]
            fig_sankey = go.Figure().update_layout(title_text="Analysis de Rutas (Fuente -> Evento) - No data")
            sankey_explanation = "No data suficientes para el diagrama de Sankey."
            ...
            if not df_source_event.empty:
                ...
                if not df_sankey_data.empty:
                    ...
                    if source_nodes:
                        ...
                        fig_sankey.update_layout(title_text="Analysis de Rutas de Usuario (Fuente/Medio -> Evento Clave)", font_size=12, height=700)
            ...
            if total_visits_funnel > 0 or (not df_source_event.empty and not df_sankey_data.empty and source_nodes):
                ...
                ai_insight_text = "No data suficientes para analizar los funnels o las rutas Sankey."

            return html.Div([
                funnels_section,
                html.Hr(className="my-4"),
                html.Div(sankey_content),
                create_ai_insight_card('funnels-ga-ai-insight-visible', title="💡 Insight Diagnóstico y Acción Action (Funnels Funnels & Rutas Paths)"),
            ...
            fig_temporal = go.Figure().update_layout(title='Descomposición Temporal y Anomalías (Insufficient data)')
            if df_acq_ts.empty or len(df_acq_ts) < 14:
                ...
                if len(dff_ts) >= 14:
                    ...
                    try:
                        ...
                        context_temporal = f"Analysis de descomposición temporal. Tendencia promedio: {decomposition.trend.dropna().mean():.2f}. Estacionalidad: Max {decomposition.seasonal.max():.2f}, Min {decomposition.seasonal.min():.2f}. Anomalías detectadas: {len(anomalies)}."
            ...
            return html.Div([
                ...
                create_ai_insight_card('temporal-ga-ai-insight-visible', title="💡 Insight Diagnóstico y Acción Action (Temporal)"),
            ...
            return html.Div([
                ...
                create_ai_insight_card('correlations-ga-ai-insight-visible', title="💡 Insight Diagnóstico y Acción Action (Correlations)"),
            ...
            fig_cohort = go.Figure().update_layout(title="Analysis de Cohorts (Datos insuficientes)")
            cohort_explanation_md = "Insufficient data para el análisis de cohortes."
                ...
                try:
                    ...
                    if not cohort_pivot.empty:
                        ...
                        if not retention_matrix_display.empty:
                            ...
                            fig_cohort.update_layout(title="Analysis de Cohorts – Retención de Usuarios (%)", xaxis_title="Días Desde la Primera Sesión", yaxis_title="Fecha de Primera Sesión (Cohorte)"); fig_cohort.update_xaxes(type='category'); fig_cohort.update_yaxes(type='category', tickformat='%Y-%m-%d')
                            cohort_explanation_md = "**Interpretación Cohorts:** Agrupa usuarios por fecha de 1ra visita y rastrea su retención. Ayuda a entender cuán bien retienes usuarios y el impacto de cambios. Filas=Cohorts, Columnas=Días desde 1ra visita, Color/Número=% Retención."
                            context_cohort = f"Analysis de Cohorts: Retención promedio Día 1: {retention_matrix_display.iloc[:, 1].mean() if len(retention_matrix_display.columns) > 1 else 'N/A':.1f}%. Retención Día 7: {retention_matrix_display.iloc[:, 7].mean() if len(retention_matrix_display.columns) > 7 else 'N/A':.1f}%."
            ...
            return html.Div([
                ...
                create_ai_insight_card('cohort-ga-ai-insight-visible', title="💡 Insight Diagnóstico y Acción Action (Cohorts)"),
    ...
    for visible_id, data_id in zip(ga_ai_insight_visible_ids, ga_ai_insight_data_ids):
        ...
        def update_ga_ai_card_generic(ai_text):
            default_no_data_msg = "Analysis IA no disponible o datos insuficientes."
            specific_no_data_msgs = [
                "Insufficient data para un análisis detallado.", "No data de Sankey para analizar.",
                "No data suficientes para analizar los funnels.", "Se necesitan al menos 14 días de datos para el análisis temporal.",
                "Error al procesar datos para análisis temporal", "Matriz de retención vacía después del procesamiento.",
                "No se pudo construir la tabla pivote para cohortes.", "Insufficient data para el análisis de cohortes.",
