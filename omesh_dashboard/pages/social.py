...
from ..settings import FACEBOOK_ID, INSTAGRAM_ID
from ..ai import get_openai_response
from ..ui.components import create_ai_insight_card, create_ai_chat_interface, add_trendline, generate_wordcloud

def layout() -> html.Div:
    return html.Div([
        dcc.Tabs(id="social-subtabs", value="general_sm", children=[
            dcc.Tab(label="General Metrics", value="general_sm"),
            dcc.Tab(label="Engagement IG", value="engagement_sm"),
            dcc.Tab(label="Wordmap", value="wordmap_sm"),
            dcc.Tab(label="Top Posts", value="top_posts_sm"),
        ]),
        html.Div(id="social-subtabs-content"),
    ])
...
def register_callbacks(app):
    """Register callbacks for the Social Media section."""
    ...
    def render_social_subtab_content(subtab_sm, start_date, end_date):
        ...
        ai_insight_text = "Insufficient data for AI analysis."
        ...
        no_data_sm_msg = html.Div([html.P("No social-media data for the selected period."), create_ai_insight_card(f'{subtab_sm}-ai-insight-visible'), html.Div(default_no_data_ai_text, id=f'{subtab_sm}-ai-insight-data', style={'display':'none'})])
        ...
        if subtab_sm == 'general_sm':
            ...
            return html.Div([
                ...
                create_ai_insight_card('general-sm-ai-insight-visible', title='💡 Insight & Action (SM Overall Metrics)'),
            ...
            if df_ig.empty or not all(k in df_ig for k in ['media_type', 'engagement', 'reach']):
                return html.Div([html.P("No data suficientes de Instagram para analizar engagement."), create_ai_insight_card('engagement-sm-ai-insight-visible'), html.Div(default_no_data_ai_text, id='engagement-sm-ai-insight-data', style={'display':'none'})])
            ...
            if not eng_by_type.empty:
                ...
                ai_insight_text = "No data de engagement para analizar."

            return html.Div([
                dbc.Row([dbc.Col(dcc.Graph(figure=fig_eng_sum), md=6), dbc.Col(dcc.Graph(figure=fig_eng_rate), md=6)]),
                create_ai_insight_card('engagement-sm-ai-insight-visible', title="💡 Insight Diagnóstico y Acción Action (Engagement IG)"),
                html.Div(ai_insight_text, id='engagement-sm-ai-insight-data', style={'display': 'none'}),
                create_ai_insight_card('engagement-sm-ai-insight-visible', title='💡 Insight                 create_ai_chat_interface('engagement_sm')
 Action (Engagement IG)'),
            ])
            ...
            if wordcloud_src:
                ...
                ai_insight_text = "No text in posts to generate the wordmap or an AI analysis."

            return html.Div([
                html.Img(src=wordcloud_src, style={'width': '100%', 'maxWidth': '800px', 'display': 'block', 'margin': 'auto'}) if wordcloud_src else html.P('Could not generate the Wordmap.'),
                create_ai_insight_card('wordmap-sm-ai-insight-visible', title='💡 Insight & Action (Wordmap)'),
            ...
            return html.Div([
                ...
                create_ai_insight_card('top-posts-sm-ai-insight-visible', title="💡 Insight Diagnóstico y Acción Action (Top Posts)"),
    ...
    for visible_id, data_id in zip(sm_ai_insight_visible_ids, sm_ai_insight_data_ids):
        ...
        def update_sm_ai_card_generic(ai_text):
            ...
            specific_no_data_msgs = [
                ...
                "No data de engagement para analizar.",
                "No text in posts to generate the wordmap or an AI analysis."
            ]
